...
public class GameplayController : MonoBehaviour
{
    ...
    private IEnumerator OnGameStart()
    {
        ...
        {
            if (SelectedStage.istance.story && SelectedStage.istance.stageNumber == 1)
            {
                screenBlurr.gameObject.SetActive(true);
                TutorialController.instance.enableDialogBox(0);
                yield return new WaitUntil((() => Input.anyKeyDown && !Pause.paused));
            ...
            if (SelectedStage.istance.story && SelectedStage.istance.stageNumber == 2)
            {
                screenBlurr.gameObject.SetActive(true);
                TutorialController.instance.enableDialogBox(0);
                yield return new WaitUntil((() => Input.anyKeyDown && !Pause.paused));
    ...
    {
        score += (int) (scoreMultiplier * ((notesHandler.notesCollected - _notesCollectedLastCall) * 400 + 
